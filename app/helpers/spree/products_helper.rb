module Spree
  module ProductsHelper
    ...
    def product_what_included(product)
      what_included = if Spree::Config[:show_raw_product_what_included]
                      product.what_included
                    else
                      product.what_included.to_s.gsub(/(.*?)\r?\n\r?\n/m, '<p>\1</p>')
                    end
      what_included.blank? ? Spree.t(:product_has_no_what_included) : raw(what_included)
    end

    def line_item_what_included_text what_included_text
      if what_included_text.present?
        truncate(strip_tags(what_included_text.gsub('&nbsp;', ' ').squish), length: 100)
      else
        Spree.t(:product_has_no_what_included)
      end
    end


        def product_additional_info(product)
      additional_info = if Spree::Config[:show_raw_product_additional_info]
                      product.additional_info
                    else
                      product.additional_info.to_s.gsub(/(.*?)\r?\n\r?\n/m, '<p>\1</p>')
                    end
      additional_info.blank? ? Spree.t(:product_has_no_additional_info) : raw(additional_info)
    end

    def line_item_additional_info_text additional_info_text
      if additional_info_text.present?
        truncate(strip_tags(additional_info_text.gsub('&nbsp;', ' ').squish), length: 100)
      else
        Spree.t(:product_has_no_additional_info)
      end
    end
